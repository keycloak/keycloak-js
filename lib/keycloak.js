// @ts-check
/**
 * @import {Acr, KeycloakAccountOptions, KeycloakAdapter, KeycloakAdapterContext, KeycloakConfig, KeycloakError, KeycloakFlow, KeycloakInitOptions, KeycloakLoginOptions, KeycloakLogoutOptions, KeycloakPkceMethod, KeycloakProfile, KeycloakRegisterOptions, KeycloakResourceAccess, KeycloakResponseMode, KeycloakResponseType, KeycloakRoles, KeycloakTokenParsed, OpenIdProviderMetadata} from "./keycloak.ts"
 */
import { createCordovaAdapter } from "./adapters/cordova.js"
import { createCordovaNativeAdapter } from "./adapters/cordova-native.js"
/*
...
export default class Keycloak {
  ...
  init = async (initOptions = {}) => {
    ...
    } else if (typeof initOptions.adapter === 'function') {
      // Adapter is a factory function - pass it the context
      this.#adapter = initOptions.adapter(this.#createAdapterContext())
  ...
  /**
   * Creates an adapter context object that provides controlled access to internal methods.
   * This context is passed to adapter factory functions to enable them to interact with
   * Keycloak core functionality without exposing these methods as public API.
   *
   * @returns {KeycloakAdapterContext} Adapter context with methods for authentication flows
   */
  #createAdapterContext() {
    return {
      createLoginUrl: /** @type {KeycloakAdapterContext['createLoginUrl']} */ (
        (options) => this.createLoginUrl(options)
      ),
      createLogoutUrl:
        /** @type {KeycloakAdapterContext['createLogoutUrl']} */ (
          (options) => this.createLogoutUrl(options)
        ),
      createRegisterUrl:
        /** @type {KeycloakAdapterContext['createRegisterUrl']} */ (
          (options) => this.createRegisterUrl(options)
        ),
      createAccountUrl: () => this.createAccountUrl(),
      clearToken: () => this.clearToken(),
      processCallback:
        /** @type {KeycloakAdapterContext['processCallback']} */ (
          async (url) => {
            const callback = this.#parseCallback(url)
            return await this.#processCallback(callback)
          }
        ),
      redirectUri: this.redirectUri || "http://localhost",
    }
  }
  ...
  #loadAdapter (type) {
    ...
    if (type === 'cordova') {
      ...
      return createCordovaAdapter(this.#createAdapterContext())
    }

    if (type === 'cordova-native') {
      this.#loginIframe.enable = false
      return createCordovaNativeAdapter(this.#createAdapterContext())
    }
