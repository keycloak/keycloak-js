  ...
  async #processInit (initOptions) {
    ...
    if (callback && callback.valid) {
      ...
      if (callback.redirectUri) {
        if (new URL(callback.redirectUri).hash) {
          window.history.replaceState(window.history.state, '', stripHash(callback.redirectUri))
          window.location.assign(callback.redirectUri)
        } else {
