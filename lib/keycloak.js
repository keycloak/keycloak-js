...
export default class Keycloak {
  ...
  #parseCallbackParams (paramsString, supportedParams) {
    // Check if there's an application route path before the OAuth params.
    // Path-style fragments start with '/' and OAuth params are key=value pairs.
    let routePath = ''
    let queryString = paramsString

    if (paramsString.startsWith('/')) {
      // Find where parameters start (either after & or ?)
      const ampIndex = paramsString.indexOf('&')
      const qMarkIndex = paramsString.indexOf('?')

      // Determine the first occurrence of a separator (& or ?)
      const validIndices = [ampIndex, qMarkIndex].filter(idx => idx > 0)
      const paramsStartIndex = validIndices.length > 0 ? Math.min(...validIndices) : -1

      if (paramsStartIndex > 0) {
        routePath = paramsString.substring(0, paramsStartIndex)
        queryString = paramsString.substring(paramsStartIndex + 1)
      } else {
        // No params, just a route
    ...
    // Reconstruct: route + remaining params
    let remainingParamsString = params.toString()
    if (routePath && remainingParamsString) {
      // Determine the right separator: if original had '?', keep '?', otherwise use '&'
      const qMarkPos = paramsString.indexOf('?')
      const ampPos = paramsString.indexOf('&')
      const hasQuestionFirst = qMarkPos !== -1 && (ampPos === -1 || qMarkPos < ampPos)
      const separator = hasQuestionFirst ? '?' : '&'
      remainingParamsString = routePath + separator + remainingParamsString
    } else if (routePath) {
      remainingParamsString = routePath
    }
