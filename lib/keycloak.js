...
export default class Keycloak {
  ...
  secureFetch = async (url, init = {}) => {
    ...
    if (dpopProvider && this.authenticated && this.token) {
      ...
      const requestUrl = new URL(request?.url ?? String(url), document.baseURI)
      const method = init.method ?? request?.method ?? 'GET'
      const baseHeaders = new Headers(init.headers ?? request?.headers)

      const existingAuth = baseHeaders.get('Authorization')
      ...
      // A Request body can only be sent once, so keep a clone in case the nonce retry below needs to resend it.
      const retryInput = request !== null && request.method !== 'GET' && request.method !== 'HEAD' ? request.clone() : url
      ...
      if (resp.status === 401 && newNonce) {
        ...
        if (wwwAuth.includes('DPoP') && wwwAuth.includes('error="use_dpop_nonce"')) {
          ...
          const retryResp = await fetch(retryInput, { ...init, headers: retryHeaders })
