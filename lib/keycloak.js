...
export default class Keycloak {
  ...
  #parseCallbackParams (paramsString, supportedParams) {
    ...
    if (paramsString.startsWith('/')) {
      ...
      if (ampIndex > 0 && (qMarkIndex === -1 || ampIndex < qMarkIndex)) {
        paramsStartIndex = ampIndex
      } else if (qMarkIndex > 0) {
        paramsStartIndex = qMarkIndex
      }
    ...
    if (routePath && remainingParamsString) {
      ...
      const separator = paramsString.includes('?') && paramsString.indexOf('?') < (paramsString.indexOf('&') !== -1 ? paramsString.indexOf('&') : Infinity) ? '?' : '&'
